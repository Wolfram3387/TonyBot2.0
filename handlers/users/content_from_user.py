...
@dp.message_handler(IsUser(), IsPrivate(), content_types=['photo', 'document', 'text'], state=[
...
@dp.message_handler(IsUser(), IsPrivate(), content_types=['text'], state=[
...
@dp.message_handler(IsUser(), IsPrivate(), content_types=['text'], state='getting_variant_title')
async def send_statistic(message: types.Message, state: FSMContext):
    if message.text == UserButtons.cancel:
        await state.finish()
...
@dp.message_handler(IsUser(), IsPrivate(), content_types=['document', 'photo', 'text'], state='other_files_to_check')
async def send_statistic(message: types.Message, state: FSMContext):
    content_type = message.content_type
    text = message.text
    user_data = await state.get_data()
...
@dp.message_handler(IsUser(), IsPrivate(), content_types=['document', 'photo', 'text'], state='oge_files_input')
async def send_statistic(message: types.Message, state: FSMContext):
    content_type = message.content_type
